...
@Component({
  ...
  templateUrl: './users-consultar.component.html',
  styleUrl: './users-consultar.component.css'
})
export class UsersConsultarComponent {
  pages: Menus[] = [];
  ...
  modulo: string = 'usuarios';
    ...
    constructor(private auth: AuthServices, private messageService: MessageService, private ms: UsersService, private router: Router) {}
    ...
    async loadPages(event: TableLazyLoadEvent)  {
      ...
      let usersResponse = await this.ms.getOnDemand('users', page, pageSize, '', apis.Seguridad);
      if (usersResponse) {
        let result: ResponseModel = usersResponse;
        if (result.error) {
        ...
        this.pages = data.value as Users[];
    ...
    edit(page: any) {
      this.router.navigate([`/usuarios-editar/${page.id}`]);
    }
  
    async onToggleChange(data: any) {
      let menu: Users = data;
      let responseActivation = await this.ms.putActivateDeactivate('users', data.id, apis.Seguridad);
    ...
    onHide(event: any): void {
      ...
      this.visible = false;
      if (event == true) {
        this.ms.delete('users', this.idEliminar, apis.Seguridad).then((response) => {
          if (response) {
