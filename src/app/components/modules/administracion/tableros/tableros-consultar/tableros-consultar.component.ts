...
@Component({
  selector: 'app-tableros-consultar',
  standalone: true,
  templateUrl: './tableros-consultar.component.html',
  styleUrl: './tableros-consultar.component.css',
...
export class TablerosConsultarComponent {
  pages: Tableros[] = [];
    ...
    async loadPages(event: TableLazyLoadEvent)  {
      ...
      let menusResponse = await this.ms.getOnDemand('tableros', page, pageSize, '', apis.Administrador);
      if (menusResponse) {
        ...
        this.pages = data.value as Tableros[];
    ...
    agregar() {
      this.router.navigate([`/tableros-crear`]);
    }
  
    edit(page: any) {
      this.router.navigate([`/tableros-editar/${page.id}`]);
    }
  
    async onToggleChange(data: any) {
      let menu: Menus = data;
      let responseActivation = await this.ms.putActivateDeactivate('tableros', data.id, apis.Administrador);
    ...
    onHide(event: any): void {
      ...
      this.visible = false;
      if (event == true) {
        this.ms.delete('tableros', this.idEliminar, apis.Administrador).then((response) => {
          if (response) {
