...
@Component({
  selector: 'app-footerNormatividad-consultar',
  standalone: true,
  templateUrl: './footerNormatividad-consultar.component.html',
  styleUrl: './footerNormatividad-consultar.component.css',
...
export class FooterNormatividadConsultarComponent  {
  @ViewChild('menu') menu!: Menu;
  pages: FooterNormatividad[] = [];
  footerNormatividad: FooterNormatividad | null = null;
  ...
  modulo: string  = 'FooterNormatividad';
    
  constructor(private auth: AuthServices, private messageService: MessageService, private ms: FooterNormatividadService, private router: Router) {}
  ...
  async loadPages(event: TableLazyLoadEvent)  {
    ...
    let menusResponse = await this.ms.getOnDemand('footerNormatividad', page, pageSize, '', apis.Administrador);
    if (menusResponse) {
      ...
      this.pages = data.value as FooterNormatividad[];
  ...
  agregar() {
    this.router.navigate([`/footerNormatividad-crear`]);
  }

  edit(page: any) {
    this.router.navigate([`/footerNormatividad-editar/${page.id}`]);
  }

  async onToggleChange(data: any) {
    this.footerNormatividad = data;
  ...
  async onHide(event: any): Promise<void> {
    ...
    if (event == true) {
      if(this.idEliminar == 0){
        let responseActivation = await this.ms.putActivateDeactivate('footerNormatividad', this.footerNormatividad?.id ?? 0, apis.Administrador);
        if (responseActivation) {
          ...
          if (result.error) {
            ...
            if(this.footerNormatividad?.estado == true){
              const icono = document.getElementById(`icono-${this.footerNormatividad?.id}`) as HTMLImageElement;
              icono.src = 'iconos/disabled.png';
              this.messageService.add({ severity: 'success', summary: 'Éxito', detail: 'Registro inactivado correctamente' });
            } else {
              const icono = document.getElementById(`icono-${this.footerNormatividad?.id}`) as HTMLImageElement;
        ...
        this.ms.delete('footerNormatividad', this.idEliminar, apis.Administrador).then((response) => {
          if (response) {
  ...
  cambiarOrden(id: number, tipo: string) {
    this.ms.putUpDown('footerNormatividad', `${id}/${tipo}`, apis.Administrador).then((response) => {
      if (response) {
        let result: ResponseModel = response;
        if (result.error) {
          this.messageService.add({ severity: 'error', summary: 'Error', detail: result.dataError.message });
