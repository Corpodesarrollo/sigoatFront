...
export class FooterInformacionInstitucionalService extends MethodsService {
  ...
  async getList(): Promise<Parametricas[]> {
    let response = await this.getAll('footerInformacionInstitucional', apis.Administrador);
    if(!response?.error){
