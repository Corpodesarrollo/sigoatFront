...
export class EnlacesInteresService extends MethodsService {
  ...
  async getList(): Promise<Parametricas[]> {
    let response = await this.getAll('enlacesInteres', apis.Administrador);
    if(!response?.error){
  ...
  public async getAllActiveById(endpoint: string, id: number, api: string): Promise<ResponseModel | null> {
      const url = `${endpoint}/getAllActiveById/${id}`;
      return new Promise((resolve) => {
          this.repos.getWithOutParameters(url, api).subscribe({
              next: (data: any) => {
                  resolve(data);
